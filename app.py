...
app.config['MYSQL_DATABASE_PASSWORD'] = 'burada-sıfre-var'
...
def bad_request(error):
    return make_response(jsonify({'serviceCode': 1, 'data': None, 'exception':{'exceptionCode':400, \
    'exceptionMessage': 'Bad request'}}),400)

@app.errorhandler(404)
def not_found(error):
    return make_response(jsonify({'serviceCode': 1, 'data': None, 'exception':{'exceptionCode':404, \
    'exceptionMessage': 'Not found'}}),404)

@app.errorhandler(405)
def not_found(error):
    return make_response(jsonify({'serviceCode': 1, 'data': None, 'exception':{'exceptionCode':405, \
    'exceptionMessage': 'Method not allowed'}}),405)
